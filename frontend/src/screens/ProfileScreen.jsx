...
const ProfileScreen = () => {
    ...
    const {data : orders,isLoading,error}=useGetMyOrdersQuery();//use the useGetMyOrdersQuery hook to get the user orders   
            ...
            <Table striped hover responsive className='table-sm'>
                ...
                <tbody>
                    {orders.map((order)=>(
                        <tr key={order._id}>
                           <td>{order._id}</td>
                           <td>{order.paidAt?.substring(0,10) || 'N/A'}</td>
                           <td>{order.totalPrice}</td>
                            <td>
                                {order.isPaid ? (order.paidAt?.substring(0,10) || 'N/A' 
                            ):(
                                <FaTimes style={{color:'red'}}/>
                            )}
                            </td>
                            <td>
                                {order.isDelivered ? (order.deliveredAt?.substring(0,10) || 'N/A'
                            ):(
                                <FaTimes style={{color:'red'}}/>
                            )}
                            </td>
